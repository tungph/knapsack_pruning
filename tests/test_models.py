...
def test_model_default_cfgs(model_name, batch_size):
    ...
    if all(x <= MAX_FWD_FEAT_SIZE for x in input_size) and not any(
    ...
    assert any(
        k.startswith(classifier) for k in state_dict.keys()
    ), f'{classifier} not in model params'
    assert any(
        k.startswith(first_conv) for k in state_dict.keys()
